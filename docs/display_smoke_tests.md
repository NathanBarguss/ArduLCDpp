...
|----|----------|-----------|--------------------|-----------------|-------|
| T1 | Power-on banner | Reset board; observe boot text before host starts streaming. | `"ArduLCD Ready"` (row 0) and `"Waiting for host..."` (row 1 when available) centered with backlight set to `STARTUP_BRIGHTNESS`. Banner clears automatically after the first serial byte. | Same banner intensity/position; OLED may emulate backlight via contrast or stay static if not supported. | Ensures factory backlight + `display.begin()` run before serial traffic. |
