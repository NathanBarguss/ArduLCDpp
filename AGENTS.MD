...
2. Populate each README with the purpose, naming pattern `TYPE-YYYYMMDD-short-slug.md`, and the markdown headers defined later in this document. That way, a brand-new contributor can open any README and immediately know how to author a compliant entry without rereading `AGENTS.MD`.
3. Confirm the workspace has write access so you can add or move bug, feature, or research files. The workflow requires creating new markdown artifacts and moving them into `RESOLVED/` archives; lack of write access blocks that process.
4. If you are unsure what to place inside a README, mirror the templates defined below:
